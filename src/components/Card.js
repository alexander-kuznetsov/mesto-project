export class Card {
    ...
    _createElement() {
        ...
        this._element  = cardTemplate.querySelector('.place');
    ...
    _setCardContent() {
        this._element.id = this._data._id; //задаем идентификатор элементу place
        ...
        this._buttonLikeElement = this._element.querySelector('.button__like');
        if (isLiked) {
            this._buttonLikeElement.classList.add('button__like_active');
        }
        ...
        this._placeLikeCountElem = this._element.querySelector('.place__like-count');
        this._placeLikeCountElem.textContent =
            Array.from(this._data.likes).length.toString();

        this._buttonRemoveElement = this._element.querySelector('.button__remove');
        if (this._userId !== this._data.owner._id) {
            this._buttonRemoveElement.classList.add('button_visible_hidden');
        }
    }
    ...
    _setEventListeners() {
        ...
        this._buttonLikeElement
            .addEventListener('click', _ => {
                const method = this._buttonLikeElement.classList
                ...
                this._addOrDeleteLike(this._data._id, method)
                    .then(cardInfo => {
                        ...
                        this._buttonLikeElement.classList.toggle('button__like_active');
                    }).catch(err => console.log(err));
            });

        //Добавляем обработчик для открытия попапа по клику на картинку
        const cardImage = this._element.querySelector('.place__image-link');
